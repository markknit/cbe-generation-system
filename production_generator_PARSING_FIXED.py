"""
Production CBE Lesson Generator - PARSING BUG FIXED
The issue was in section extraction - now completely rewritten
"""
import os
import sys
import re
...
class ProductionLessonGenerator:
    """Production generator with FIXED section parsing"""
    
    def __init__(self):
        print("\n" + "="*70)
        print("PRODUCTION CBE LESSON GENERATOR - PARSING BUG FIXED")
        print("="*70 + "\n")
    ...
    def extract_sections(self, content: str) -> Dict[str, str]:
        """Extract sections using regex - FIXED VERSION"""
        
        sections = {}
        
        # Use regex to find each section
        # Pattern: ===SECTION X: ... === (up to next section or end)
...
Content: [PCIs]"""
    ...
    def generate_lesson_content(self, subject: str, grade: str, strand: str,
        ...
        """Generate lesson with explicit Section C requirements"""
        
        prompt = f"""Generate COMPLETE CBE lesson for Kenya. CRITICAL: Must include Section C with 7 rows!
...
ROW 1 - PREDICT:
Experience: [40+ words describing PREDICT phase]
Resources: [Materials with [ARES VIDEO: ...]]
Teacher: [Questions with "PROBING:", "WAIT TIME: 10 seconds"]
...
Teacher: [Facilitation with "PROBING:", "WAIT TIME:"]
...
Assessment: [☐ Quality criteria, Exit Ticket]

ROW 4 - DQB:
Experience: [Class generates questions about {topic}]
Resources: [Chart paper, markers, [ARES VIDEO: DQB]]
Teacher: [Establishing DQB, record ALL questions]
Sensemaking: [List 4-5 example questions]
Assessment: [☐ DQB Started, note for Summary Table]

ROW 5 - INITIAL MODEL:
Experience: [Students draw initial model of {topic}]
Resources: [Drawing materials, [ARES SIMULATION: Model]]
Teacher: [Facilitate, "no wrong models at this stage"]
Sensemaking: [Model creation, Think-Pair-Share]
Assessment: [☐ Model quality, collect for later]

ROW 6 - KENYA CONTEXT:
Experience: [How does {topic} help Kenya? KSh costs, careers]
Resources: [Kenya materials, photos, [ARES VIDEO: Kenya]]
Teacher: [Discuss Kenyan applications]
Sensemaking: [Think-Pair-Share on Kenya uses]
Assessment: [☐ Kenya connections, ☐ Career awareness]

ROW 7 - MODEL REVISION:
Experience: [Revise initial models with evidence]
Resources: [Initial models, revision materials]
Teacher: [Compare models, "scientists revise models"]
Sensemaking: [Before/after comparison]
Assessment: [☐ Improvements, ☐ Labels, ☐ Evidence-based]
...
[Summary prompt for students' table]

Generate the complete lesson now with ALL sections, especially Section C with 7 rows!"""
    ...
    def create_word_document(self, content: str, overview_content: str,
        ...
        """Create Word document with FIXED section parsing"""
        ...
        # Add lesson
        self._add_lesson_fixed(doc, content, subject, grade, substrand, topic, number, total_lessons)
    ...
    def _add_substrand_overview(self, doc, content: str, substrand: str):
        ...
        doc.add_paragraph('Sub-Strand Overview').alignment = WD_ALIGN_PARAGRAPH.CENTER
        doc.add_paragraph()
        ...
        for component in ['Specific Learning Outcomes', 'Key Inquiry Questions', 
            ...
            if f'Component: {component}' in content:
                ...
                # Extract text between this component and next
    ...
    def _add_lesson_fixed(self, doc, content: str, subject: str, grade: str,
                         substrand: str, topic: str, number: int, total_lessons: int):
        """Add lesson with FIXED section extraction"""
        ...
        # Extract sections using FIXED method
    ...
    def _add_section_b(self, doc, section_text):
        ...
        for i, (label, marker) in enumerate(items, 1):
            if marker in section_text:
                ...
                # Find end - next marker or double newline
                end = len(section_text)
    ...
    def _add_section_c(self, doc, section_text):
        """Add Implementation Framework - THE CRITICAL SECTION"""
        doc.add_heading('C. Lesson Implementation Framework', 2)
        
        # Count rows
        rows = section_text.split('ROW ')[1:]  # Skip text before first ROW
        row_count = len(rows)
        
        print(f"    → Creating table with {row_count} content rows")
        
        if row_count == 0:
            doc.add_paragraph("⚠ ERROR: No rows detected in Section C")
            doc.add_paragraph(section_text[:500])
            return
        
        # Create table with header + content rows
        table = doc.add_table(rows=row_count + 1, cols=5)
        table.style = 'Light Grid Accent 1'
        
        # EQUAL column widths - 2.0 inches each
        ...
        for row_idx, row_text in enumerate(rows, 1):
            ...
            # Extract each column using markers
            columns_data = {
                'Experience': '',
                'Resources': '',
                'Teacher': '',
    ...
    def generate_complete_lesson(self, subject: str, grade: str, strand: str,
        ...
        if '===SECTION C:' in content:
            print("  ✓ Section C present in generated content")
        else:
            print("  ✗ WARNING: Section C missing from generated content!")
...
def main():
    ...
    print("\nNow analyze this file with:")
    print("  python3 analyze_docx.py")
