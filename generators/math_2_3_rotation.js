'use strict';
/**
 * Generator: Mathematics Grade 10 — Sub-Strand 2.3: Rotation
 *
 * No teacher template exists — phenomenon and storyline invented from KICD curriculum.
 * Phenomenon: The spinning blades of the Ngong Hills Wind Farm (Kenya's largest wind project),
 *   and the rotating gears inside a Nairobi matatu dashboard clock.
 *
 * Outputs (3 formats each):
 *   data/outputs/docx/Grade 10 Math/Math 2.3 Rotation/docx/
 *   data/outputs/docx/Grade 10 Math/Math 2.3 Rotation/html/
 *   data/outputs/docx/Grade 10 Math/Math 2.3 Rotation/gfm/
 */
...
const BASE = path.join(__dirname, '..', 'data', 'outputs', 'docx', 'Grade 10 Math', 'Math 2.3 Rotation');
...
async function buildSoW() {
  const body = [
    ...titleBlock(
      ...
      'CBE Phenomenon-Driven Lesson Sequence — Sub-Strand 2.3 (6 Lessons)'
    ),
    SPACE(),
...
async function buildFinalExplanation() {
  ...
  const headerRows = [
    fullHeader('FINAL EXPLANATION: ROTATION', C.darkBlue, 'FFFFFF', SZ_H, 2),
  ...
  const instrRows = [
    ...
    new TableRow({ children: [cell(
      'You have completed all 6 lessons of Rotation. Write your COMPLETE EXPLANATION for the driving question:\n' +
      '"What mathematical rules describe how a point rotates around a fixed centre — and how do engineers use these rules?"\n\n' +
      'USE: Your Summary Table, Rotation Model, Design Challenge work, and all class activities.\n\n' +
      'YOUR EXPLANATION MUST INCLUDE:\n' +
      '• The three elements that define a rotation (centre, angle, direction)\n' +
      '• Coordinate rules for rotation about the origin (all four standard angles)\n' +
      '• The three-step method for rotation about any point\n' +
      '• The three properties of rotation (isometry)\n' +
      '• The method for finding centre and angle from object and image\n' +
      '• At least TWO Kenyan engineering examples using rotation\n\n' +
  ...
  const parts = [
    { title: 'PART 1: DEFINING ROTATION',
      prompt: 'Define rotation. State the three parameters that fully describe any rotation. Use the Ngong Hills wind turbine as an example — give values for all three parameters.',
      answer:
        'Rotation is a transformation that turns every point of a shape around a fixed point (the centre of rotation) by a given number of degrees (the angle of rotation) in a given direction (clockwise or anticlockwise). The centre itself does not move.\n\n' +
        'Three parameters:\n' +
        '1. Centre of rotation — the fixed point that all other points revolve around.\n' +
        '2. Angle of rotation — the number of degrees each point turns.\n' +
        '3. Direction — clockwise (CW) or anticlockwise (ACW).\n\n' +
        'Ngong Hills Wind Turbine example:\n' +
        '• Centre: the hub (mechanical axle) of the turbine — it never moves.\n' +
        '• Angle: 120° per blade-to-blade rotation (360° ÷ 3 equally-spaced blades = 120° between blades). In one full revolution each blade turns 360°.\n' +
        '• Direction: anticlockwise as viewed from the front (the side facing the prevailing wind), consistent with most horizontal-axis wind turbines in Kenya.' },

    { title: 'PART 2: COORDINATE RULES — ROTATION ABOUT THE ORIGIN',
      prompt: 'State the coordinate rules for 90° ACW, 180°, and 270° ACW rotations about the origin. For each, explain WHY the rule works using the geometry of a circle. Give a worked example for 90° ACW.',
      answer:
        'Coordinate rules for rotation about the origin:\n\n' +
        '90° ACW: (x, y) → (−y, x).\n' +
        'WHY: a point at distance r from the origin at angle θ is (r cosθ, r sinθ). After 90° ACW it is at angle (θ + 90°): the new coordinates are (r cos(θ+90°), r sin(θ+90°)) = (−r sinθ, r cosθ) = (−y, x).\n\n' +
        '180°: (x, y) → (−x, −y).\n' +
        'WHY: half-circle rotation reverses direction on both axes. The point moves to the diametrically opposite position on the circle.\n\n' +
        '270° ACW (= 90° CW): (x, y) → (y, −x).\n' +
        'WHY: three-quarter turn anticlockwise = one-quarter turn clockwise. Algebraically, apply the 90° ACW rule three times: (x,y) → (−y,x) → (−x,−y) → (y,−x).\n\n' +
        '360°: (x, y) → (x, y) — full circle, identity transformation.\n\n' +
        'Worked example — 90° ACW, point P(3, 4):\n' +
        'P(3, 4) → P\'(−4, 3).\n' +
        'Verification:\n' +
        '• Distance from origin: |OP| = √(3²+4²) = 5; |OP\'| = √(4²+3²) = 5 ✓ (distances preserved).\n' +
        '• Angle ∠POP\': gradient OP = 4/3; gradient OP\' = 3/(−4) = −3/4. Product = (4/3)(−3/4) = −1 ✓ (perpendicular → 90° angle between them).' },

    { title: 'PART 3: ROTATION ABOUT ANY POINT',
      prompt: 'Explain the three-step method for rotating about any point (a,b). Show ALL STEPS for rotating point Q(5,3) by 90° ACW about centre P(2,1). Verify your answer.',
      answer:
        'The three-step method for rotating about centre (a, b):\n\n' +
        'Step 1 — Translate so the centre becomes the origin: subtract (a, b) from all coordinates.\n' +
        'Step 2 — Apply the appropriate origin rotation rule.\n' +
        'Step 3 — Translate back: add (a, b) to all coordinates.\n\n' +
        'Worked example: Q(5, 3), 90° ACW about P(2, 1).\n\n' +
        'Step 1: Q − P = (5−2, 3−1) = (3, 2). [P is now the origin]\n' +
        'Step 2: Apply 90° ACW rule (x,y)→(−y,x): (3, 2) → (−2, 3).\n' +
        'Step 3: Add P back: (−2+2, 3+1) = (0, 4) = Q\'.\n\n' +
        'Verification (using the equal-distance property):\n' +
        '|PQ|  = √((5−2)²+(3−1)²) = √(9+4) = √13.\n' +
        '|PQ\'| = √((0−2)²+(4−1)²) = √(4+9) = √13. ✓ Distances from centre are equal, as required by Property 1 of rotation.' },

    { title: 'PART 4: PROPERTIES OF ROTATION',
      prompt: 'State the three properties of rotation. Explain what "isometry" means. Use the congruence tests (from Sub-Strand 2.2) to prove that object and image are congruent after rotation.',
      answer:
        'Three properties of rotation:\n\n' +
        '1. Distances from the centre are preserved: for every point A and its image A\', |OA| = |OA\'|, where O is the centre of rotation.\n' +
        '2. All side lengths are preserved: |AB| = |A\'B\'| for every pair of corresponding points. Rotation is an isometry.\n' +
        '3. All angles are preserved: ∠ABC = ∠A\'B\'C\' for every angle in the figure.\n\n' +
        'Isometry: a transformation that preserves all distances and angles. Rotation, reflection, and translation are all isometries. Dilation (enlargement) is NOT an isometry because it changes lengths.\n\n' +
        'Proof of congruence using SSS (from Sub-Strand 2.2):\n' +
        'Let triangle ABC rotate to triangle A\'B\'C\'.\n' +
        'By Property 2 (lengths preserved): |AB| = |A\'B\'|, |BC| = |B\'C\'|, |CA| = |C\'A\'|.\n' +
        'Three sides of △ABC equal the three corresponding sides of △A\'B\'C\'.\n' +
        'By the SSS congruence test: △ABC ≅ △A\'B\'C\'. ✓' },

    { title: 'PART 5: FINDING CENTRE AND ANGLE',
      prompt: 'Describe the perpendicular bisector method for finding the centre of rotation. Explain why two perpendicular bisectors are needed. How do you find the angle? Connect this method to the centre-finding method in Lesson 2.2 (reflection).',
      answer:
        'Perpendicular bisector method for finding the centre of rotation:\n\n' +
        '1. Identify a corresponding pair of points: vertex A and its image A\'.\n' +
        '2. Construct the perpendicular bisector of segment AA\'.\n' +
        '   (The centre must be equidistant from A and A\' — Property 1 — so it lies on this bisector.)\n' +
        '3. Identify a second pair: vertex B and image B\'. Construct the perpendicular bisector of BB\'.\n' +
        '4. The intersection of the two perpendicular bisectors is the centre of rotation O.\n' +
        '5. Finding the angle: draw OA and OA\'. Measure ∠AOA\' with a protractor — this is the angle of rotation.\n' +
        '6. Verification: construct the perpendicular bisector of a third pair CC\'. It should also pass through O.\n\n' +
        'Why two bisectors are needed: one perpendicular bisector gives a whole line of possible centre positions. A second perpendicular bisector (from a different vertex pair) is also a line of possible positions. The unique point on BOTH lines — their intersection — is the only point equidistant from all corresponding pairs. One bisector is insufficient; two give a unique answer.\n\n' +
        'Connection to Lesson 2.2 (reflection, mirror-line finding):\n' +
        'In Lesson 2.2, finding the mirror line from object + image also used the perpendicular bisector of PP\'. There, the mirror line IS the perpendicular bisector (not just a line through the intersection). Both inverse problems — finding the mirror line for reflection and finding the centre for rotation — rely on the same geometric tool: the perpendicular bisector. This is an example of mathematical unity: the same tool solves different inverse problems.' },

    { title: 'PART 6: ANSWER THE DRIVING QUESTION',
      prompt: '"How does rotation create motion and patterns that power machines and keep time in Kenya?" Write your complete answer citing evidence from ALL 6 lessons, your design challenge, and at least two Kenyan examples (wind farm, clock, gear, or other).',
      answer:
        'Rotation creates controlled, predictable motion — and that predictability is what makes it useful for engineering.\n\n' +
        'From Lesson 1 we know that every rotation is fully described by three parameters: centre, angle, and direction. The Ngong Hills turbine blades rotate anticlockwise, through 120° at a time, about the fixed hub. The clock second hand rotates clockwise through 6° per second (360° ÷ 60 seconds) about the clock\'s centre pin. In both cases, knowing the three parameters means we can predict exactly where any point will be at any time.\n\n' +
        'Lessons 2 and 3 gave us the coordinate rules: 90° ACW sends (x,y) to (−y,x); 180° to (−x,−y); 270° ACW to (y,−x). The three-step method (translate centre to origin → rotate → translate back) extends this to any centre. Together, these tools mean a turbine engineer can calculate the exact coordinates of a blade tip at any moment — no physical measurement needed.\n\n' +
        'Lesson 4 established that rotation is an isometry: it preserves all distances and angles. This is why turbine blades do not deform or lengthen as they spin, and why the second hand of a clock maintains its exact length after every rotation. The SSS congruence test (from Sub-Strand 2.2) proves that the object and image are always congruent.\n\n' +
        'Lesson 5 showed that the centre of rotation can be found from any object and image using the perpendicular bisector method — the same tool used to find reflection mirror lines. This allows engineers to verify the true centre of a rotating machine from measurement alone.\n\n' +
        'In the design challenge (Lesson 6), calculating the three blade positions of the turbine (120° apart) and the twelve hour-hand positions of the clock (30° apart) demonstrated that rotation rules are directly applicable to real Kenyan engineering.\n\n' +
        'In conclusion: rotation creates the periodic, calculable, non-distorting motion that machines depend on. Equal blade spacing (120°) balances turbine forces; equal degree steps (30°/hour, 6°/second) make timekeeping precise. The mathematics of rotation — three parameters, coordinate rules, isometry, and centre-finding — is the mathematical foundation of every rotating machine in Kenya and the world.' },
  ];
  ...
  const rubricRows = [
    ...
    ...[
      ['Rotation Definition', 'All three parameters stated with turbine example; values accurate.', 'Three parameters named; turbine example partial.', 'Fewer than three parameters or no example.'],
      ['Coordinate Rules', 'All four rules (90°ACW, 180°, 270°ACW, 360°) with geometric justification and worked example.', '3–4 rules stated; geometric justification partial.', 'Fewer than 3 rules or no justification.'],
      ['Three-Step Method', 'All three steps with worked example; correct answer; verified.', 'Correct method but arithmetic error; or no verification.', 'Incorrect method or missing steps.'],
      ['Properties and Isometry', 'All three properties; isometry defined; congruence test applied correctly.', '2–3 properties; isometry defined; partial congruence connection.', 'Fewer than 2 properties or missing isometry.'],
      ['Real-World Application', 'Two Kenyan examples fully explained using specific rotation concepts from lessons.', 'Two examples mentioned but one only superficially explained.', 'Only one example or analysis is generic.'],
  ...
  const body = [
    ...titleBlock('FINAL EXPLANATION: ROTATION', 'Mathematics Grade 10 — Student Assessment Document'),
    SPACE(),
...
async function buildSummaryTable() {
  ...
  const body = [
    ...titleBlock('SUMMARY TABLE: ROTATION', 'Mathematics Grade 10 — Teacher Reference'),
    SPACE(),
    makeTable([
      fullHeader('SUMMARY TABLE: ROTATION', C.darkBlue, 'FFFFFF', SZ_H, 2),
      fullHeader('Mathematics Grade 10 — Teacher Reference (Pre-filled)', C.teal, 'FFFFFF', SZ_H, 2),
      labelRow('Sub-Strand', '2.3: Rotation', SLW),
    ...
    makeTable([
      ...
      new TableRow({ children: [cell(
        '1. Can students explain all three rotation parameters using the turbine as an example?\n\n' +
        '2. Do students understand the coordinate rules geometrically (from circle geometry) or only as formulas to memorise?\n\n' +
        '3. Three-step method: do students understand WHY translating to origin works — not just how to do it?\n\n' +
        '4. Did students connect rotation properties to congruence tests from Sub-Strand 2.2?\n\n' +
        '5. Centre-finding: did students make the parallel with Lesson 2.2 (same perpendicular bisector tool)?\n\n' +
        '6. Design challenge: did students apply rotation to real engineering with mathematical precision, or approximate visually?',
...
<title>Math 2.3: Rotation — CBE Lesson Sequence</title>
<style>
...
<body>
<h1>MATHEMATICS GRADE 10: ROTATION</h1>
<h2>CBE Phenomenon-Driven Lesson Sequence — Sub-Strand 2.3 (6 Lessons)</h2>
<table>
...
function buildGFM() {
  let md = `# Mathematics Grade 10: Rotation\n`;
  md += `## CBE Phenomenon-Driven Lesson Sequence — Sub-Strand 2.3 (6 Lessons)\n\n`;
  md += `## Sub-Strand Overview\n\n| Field | Detail |\n|---|---|\n`;
  for (const [k, v] of [
    ['Grade Level', UNIT.gradeLevel], ['Subject', UNIT.subject],
    ['Strand', UNIT.strand], ['Sub-Strand', UNIT.substrand],
    ['Total Duration', UNIT.duration], ['Key Inquiry Question', UNIT.keyInquiry],
  ...
  for (const lesson of LESSONS) {
    ...
    md += `**Duration:** ${lesson.duration}  \n**Key Inquiry:** ${lesson.slo.keyInquiry}\n\n`;
...
async function main() {
  console.log('Generating Math 2.3 Rotation...\n');

  console.log('1/3 Building docx files...');
  const sowBuf = await Packer.toBuffer(await buildSoW());
  fs.writeFileSync(path.join(OUT_DOCX, 'Math_Rotation_CBE_LessonSequence.docx'), sowBuf);
  const feBuf  = await Packer.toBuffer(await buildFinalExplanation());
  fs.writeFileSync(path.join(OUT_DOCX, 'Math_Rotation_FinalExplanation.docx'), feBuf);
  const stBuf  = await Packer.toBuffer(await buildSummaryTable());
  fs.writeFileSync(path.join(OUT_DOCX, 'Math_Rotation_SummaryTable.docx'), stBuf);
  console.log(`    Saved 3 docx files to ${OUT_DOCX}\n`);

  console.log('2/3 Building HTML...');
  fs.writeFileSync(path.join(OUT_HTML, 'Math_Rotation_CBE_LessonSequence.html'), buildHTML(), 'utf8');
  console.log(`    Saved to ${OUT_HTML}\n`);

  console.log('3/3 Building GFM...');
  fs.writeFileSync(path.join(OUT_GFM, 'Math_Rotation_CBE_LessonSequence.md'), buildGFM(), 'utf8');
  console.log(`    Saved to ${OUT_GFM}\n`);

  console.log('Done! All 5 files generated for Math 2.3.');
}
