...
function buildResourceParagraphs(resources, phase = '') {
  ...
  if (v) {
    // Title: embedded hyperlink if direct URL available, else link to exact-name search
    ...
    if (v.exact_search_url) {
      paras.push(_metaPara(`Find by name: ${v.exact_search_url}`));
    }
    paras.push(_metaPara(`Find by topic: ${v.search_url}`));
    paras.push(_metaPara(`Topic terms: "${v.search_terms}"`));
  } else {
  ...
  if (r) {
    ...
    if (r.exact_search_url) {
      paras.push(_metaPara(`Find by name: ${r.exact_search_url}`));
    }
    paras.push(_metaPara(`Find by topic: ${r.search_url}`));
    paras.push(_metaPara(`Topic terms: "${r.search_terms}"`));
  } else {
